...
class UI {
    ...
    static eliminarLibro(evento) {
        if (evento.classList.contains('delete')) {
...
document.querySelector('#libro-form').addEventListener('submit', (e) => {
    ...
    if (titulo == '' || autor == '' || isbn == '') {
        UI.mostarAlerta('ingresa los valors de cada campo', 'danger');
    } else {
        const libro = new Libro(titulo, autor, isbn);
        Datos.agregarLibro(libro);
        UI.agregarLibroLista(libro);
        UI.mostarAlerta('se agrego un nuevo libro a la lista', 'success');
        UI.limparCampos();
...
document.querySelector('#libro-list').addEventListener('click', (e) => {
    ...
    UI.mostarAlerta('libro eleminado', 'danger');
});
